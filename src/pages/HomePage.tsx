...
function staticExamples() {
  ...
  const examples: LExample[] = [
    ...
    },
    {
      ...
      iterations: 7,
      gfxprops: { angle: 22.5, length: 45, center: [0, 0.4] }
    },  
    {
      name: "Plants with flowers",
      axiom: `A(8)`,
      productions: ["A(x){x>1}:F[-B][+B]A(x-1)", "A(x){x<=1}: BE", "B: FF"], iterations: 10,
      gfxprops: { angle: 30, length: 30, center: [0, 0.4] }
    },     {
      name: "Plant Gradient",
      axiom: "P P P P", productions: ["P: +(90) f(50) -(90) [A(rnd(0,10))]", "A(a): #(a) FFF I(40) +(rnd(0-a,a))  A(a+1) E(4)", "I(a): [-(a)B] [+(a)B]", "B:FBE(4)", "B:I(rnd(0,10))"], iterations: 25,
    ...
    },
    {
      ...
      productions: ["A(d,D){d>0}: A(d-1,D)", "A(d,D){d==0}: F(1)[+(45)A(D,D)][-(45)A(D,D)]F(1)A(0,D)", "F(a): #(a*25) F(a*1.48)"],
